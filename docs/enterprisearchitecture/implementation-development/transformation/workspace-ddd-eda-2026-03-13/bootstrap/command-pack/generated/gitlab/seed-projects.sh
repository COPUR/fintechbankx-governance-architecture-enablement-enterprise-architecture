...
EOF
  ...
  if [[ "${CREATE_MR}" == "true" ]] && command -v glab >/dev/null 2>&1; then
    glab mr create         --repo "${GITLAB_GROUP_PATH}/${repo}"         --source-branch "${SEED_BRANCH}"         --target-branch "${BASE_BRANCH}"         --title "chore(bootstrap): initialize DDD/EDA skeleton"         --description "Automated bootstrap for DDD/EDA repository skeleton and ownership metadata." >/dev/null || true
